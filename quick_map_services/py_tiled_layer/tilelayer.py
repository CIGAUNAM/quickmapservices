...
class TileLayer(QgsPluginLayer):
    ...
    def __init__(self, layerDef, creditVisibility=1):
        ...
        self.eventLoop = None
        QObject.connect(self, pyqtSignal("fetchRequest(QStringList)"), self.fetchRequest)
        if self.iface:
            QObject.connect(self, pyqtSignal("showMessage(QString, int)"), self.showStatusMessageSlot)
            QObject.connect(self, pyqtSignal("showBarMessage(QString, QString, int, int)"), self.showBarMessageSlot)
    ...
    def networkReplyFinished(self, url, error, isFromCache):
        ...
        if unfinishedCount == 0:
            self.emit(pyqtSignal("allRepliesFinished()"))
    ...
    def fetchFiles(self, urls):
        ...
        QObject.connect(self, pyqtSignal("allRepliesFinished()"), eventLoop.quit)
        ...
        self.emit(pyqtSignal("fetchRequest(QStringList)"), urls)
        ...
        QObject.disconnect(self, pyqtSignal("allRepliesFinished()"), eventLoop.quit)
    ...
    def showStatusMessage(self, msg, timeout=0):
        self.emit(pyqtSignal("showMessage(QString, int)"), msg, timeout)
    ...
    def showBarMessage(self, text, level=QgsMessageBar.INFO, duration=0, title=None):
        if PluginSettings.show_messages_in_bar():
            ...
            self.emit(pyqtSignal("showBarMessage(QString, QString, int, int)"), title, text, level, duration)
...
class TileLayerType(QgsPluginLayerType):
    ...
    def showLayerProperties(self, layer):
        ...
        QObject.connect(dialog, pyqtSignal("applyClicked()"), self.applyClicked)
        dialog.show()
    ...
    def applyProperties(self, dialog):
        ...
        layer.emit(pyqtSignal("repaintRequested()"))
